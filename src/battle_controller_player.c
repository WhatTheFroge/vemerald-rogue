...
static u8 GetDisplaySplit(u16 move, u16 displayType)
{
	...
	if (IS_TYPE_PHYSICAL(movetype) == TRUE)
		return PHYSICAL_MOVE;
	
	if (IS_TYPE_SPECIAL(movetype) == TRUE)
		return SPECIAL_MOVE; 
}
...
static u8 const* GetDisplaySplitLong(u8 split)
{
	...
	{
		case PHYSICAL_MOVE:
			return gText_Physical; 
		case SPECIAL_MOVE:
			return gText_Special; 
		default:
...
static void PlayerHandleDrawTrainerPic(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK)
    {
        ...
        {
            trainerPicId = gLinkPlayers[GetMultiplayerId()].gender + TRAINER_BACK_PIC_NONE;
        }
    }
    else
    {
...
static void PlayerHandleIntroTrainerBallThrow(void)
{
    ...
    LoadCompressedPalette(gTrainerBackPicPaletteTable[RoguePlayer_GetTrainerBackPic()].data, 0x100 + paletteNum * 16, 32);
