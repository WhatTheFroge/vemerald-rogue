...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT)
    {
        ...
        if (statusChanged == TRUE)
        {
			...
			// In generation 3, Sleep lasts 1-4 turns. 
			// gBattleMons[gEffectBattler].status1 |= STATUS1_SLEEP_TURN((Random() & 3) + 2); // 2-5 = 1-4 turns 
            if (sStatusFlagsForMoveEffects[gBattleCommunication[MOVE_EFFECT_BYTE]] == STATUS1_SLEEP)
                //gBattleMons[gEffectBattler].status1 |= STATUS1_SLEEP_TURN((Random() & 1) + 1); // 1-2 = 0-1 turns 
				gBattleMons[gEffectBattler].status1 |= STATUS1_SLEEP_TURN((Random() & 3) + 1); // 1-4 = 0-3 turns  
            else
...
static void Cmd_getexp(void)
{
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            for (viaSentIn = 0, i = 0; i < PARTY_SIZE; i++)
            {
                ...
                if (holdEffect == HOLD_EFFECT_EXP_SHARE || Rogue_ForceExpAll())
                    viaExpShare++;
            }
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (!(holdEffect == HOLD_EFFECT_EXP_SHARE || Rogue_ForceExpAll()) && !(gBattleStruct->sentInPokes & 1))
            {
            ...
            else
            {
                ...
                if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP))
                {
                    ...
                    // RogueNote: Just print team gained exp?
                    //if(firstExpGained)
                    //{
...
static void Cmd_setgraphicalstatchangevalues(void)
{
    ...
    case SET_STAT_BUFF_VALUE(3) | STAT_BUFF_NEGATIVE: // -2
    ...
    case 4:
        if (!gPaletteFade.active && gMain.callback2 == BattleMainCB2)
        {
            ...
            else
            {
                ...
                if (IsHMMove2_LearnReplaceCheck(moveId))
                {
...
static void Cmd_tryinfatuating(void)
{
    ...
    {
        if (GetMonGender(monAttacker) == GetMonGender(monTarget)
            || gBattleMons[gBattlerTarget].status2 & STATUS2_INFATUATION
            || GetMonGender(monAttacker) == MON_GENDERLESS
            || GetMonGender(monTarget) == MON_GENDERLESS)
        {
            gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
        }
        else
...
static void Cmd_rogue_caughtmon(void)
{
    bool8 discarded = gBattlescriptCurrInstr[1];
...
static void Cmd_givecaughtmon(void)
{
    ...
    Rogue_OnAcceptCaughtMon(&gEnemyParty[gBattlerPartyIndexes[gBattlerAttacker ^ BIT_SIDE]]);
