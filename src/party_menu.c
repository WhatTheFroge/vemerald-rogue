...
static bool8 DisplayPartyPokemonDataForMoveTutorOrEvolutionItem(u8 slot)
{
    ...
    else
    {
        ...
        case 2: // Evolution stone
            if (!GetMonData(currentPokemon, MON_DATA_IS_EGG) && GetEvolutionTargetSpecies(currentPokemon, EVO_MODE_ITEM_CHECK, item) != SPECIES_NONE, NULL)
                return FALSE;
...
static void CursorCb_FieldMove(u8 taskId)
{
    ...
    else
    {
        ...
        if( fieldMove == FIELD_MOVE_FLY
         || fieldMove == FIELD_MOVE_DIG
         || fieldMove == FIELD_MOVE_TELEPORT
         || fieldMove == FIELD_MOVE_SECRET_POWER
        )
        {
